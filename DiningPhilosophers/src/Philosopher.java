...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		long seconds  = (1+ sec.nextInt(11))*1000;
		try {
			sleep(seconds);
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		barriers[id].release(1);
		barriers[id].release(1);
		barriers[(id+1)%5].acquireUninterruptibly();
		barriers[(id+4)%5].acquireUninterruptibly();
		barriers[(id+2)%5].acquireUninterruptibly();
		barriers[(id+3)%5].acquireUninterruptibly();
		...
		while(true)
		{
			...
			int right = (id+1)%5;
			//test left
			if(state[left] ==HUNGRY && state[(left+4)%5] != EATING && state[(left+1)%5] != EATING) {
				table.ForkTake_GUI(left);
				state[left] = EATING;
				sem[left].release();
			}
			//test right
			if(state[right] ==HUNGRY && state[(right+4)%5] != EATING && state[(right+1)%5] != EATING)
			{
				table.ForkTake_GUI(right);
				state[right] =EATING;
				sem[right].release();
			}
			mutex.release();
